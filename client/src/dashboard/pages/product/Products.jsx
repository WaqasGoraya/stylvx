...
const Products = () => {
  ...
  const handleDelete = async(productId)=> {
    ...
    if(isConfirmed){
      try {
        const response = await axios.delete(`${import.meta.env.VITE_BASE_URL}/products/delete/${productId}`,{withCredentials:true})
  ...
  return (
    <>
      <div className="col-lg-9">
              ...
              <div className="table-responsive">
                <table className="table">
                  ...
                  {products? (products.map((product,index)=>(
                    <tr key={index}>
                      <td><ShowImage imagePath={product.image} className={"img-fluid me-2"} width={"22px"} height={"auto"} alt={product.name}/></td>
                      <td>{product.name}</td>
                      <td>${product.price}</td>
                      <td>{product.stock}</td>
                      <td>{product.category.name}</td>
                      <td><span onClick={()=>handleEdit(product._id)} style={{cursor:'pointer'}} data-bs-toggle="modal" data-bs-target="#EditModal"><img className="img-fluid me-2" src="/images/bxs_edit.png" width="22px" height="auto" alt=""/></span>
                        <span onClick={()=>handleDelete(product._id)} style={{cursor:'pointer'}}><img className="img-fluid" src="/images/delete-icon.png" width="22px" height="auto" alt=""/></span></td>
                    </tr>
                  ))):(
                  <tr>
                  <td colSpan='6' style={{textAlign:"center"}}>No Product Data Available!</td>
                  </tr>
                  )}
                  </tbody>
                </table>
        ...
        {isProductModalOpen && createPortal(<EditProduct close={closeProductModal}  id={selectedProductId} />, document.body)}
    </>
  )
}
